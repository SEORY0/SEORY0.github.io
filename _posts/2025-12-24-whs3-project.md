...
[UserMode]                              [KernelMode]
 ...
 Application                            ks.sys
     ...
     │                                       │       │
     │                                       │       ▼ 새 IRP 생성
     │                                       │       RequestorMode = KernelMode ← 문제!
     │                                       │       (사용자 데이터 포함)
     │                                       │
     │                                       ├── 새 IRP가 ksthunk.sys로 전달
     │                                       │   RequestorMode = 0 이므로
     │                                       │   검증 생략!
```
...
│                    ▼                                            │
│   [2] kCFG 우회 가젯 설정                                        │
│       └── fake 구조체 구성, 가젯 주소 설정                       │
│                    │                                            │
│                    ▼                                            │
│   [3] PreviousMode 변조                                         │
│       └── 취약한 IOCTL 호출로 PreviousMode = 0 설정              │
│       └── Arbitrary Write Primitive 확보                        │
│                    │                                            │
│                    ▼                                            │
│   [4] Token Swapping                                            │
│       └── 현재 프로세스 Token을 SYSTEM Token으로 덮어쓰기        │
│       └── PreviousMode 복구                                     │
│       └── SYSTEM 권한 쉘 실행                                   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
...
```
0x10000000: ┌────────────────────────────────────┐
            │ &PreviousMode - 0x2078  (8 bytes)  │ ← [rcx]
0x10000008: ├────────────────────────────────────┤
            │ (unused)                           │
0x10000010: ├────────────────────────────────────┤
            │ 0x00000800              (4 bytes)  │ ← [rcx + 0x10], 쓸 값
0x10000014: ├────────────────────────────────────┤
            │ 0x00100100              (4 bytes)  │ ← [rcx + 0x14]
            └────────────────────────────────────┘

실행 결과:
[[rcx] + 0x2078] = [rcx + 0x10]
→ [&PreviousMode - 0x2078 + 0x2078] = 0x00000800
→ PreviousMode = 0x00 (KernelMode)
```
